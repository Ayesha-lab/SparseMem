...
def get_unitmem(model, level, result_path, args):
    ...
    for name, bn_layer in bn_layers:
        ...
        meanout = (torch.mean(torch.sort(activations, dim=0)[0][:-1], dim=0)).round(decimals=6)

        denominator = maxout + meanout
        denominator[denominator == 0] += 0.001
        selectivity = (maxout - meanout) / denominator
        ...
        unit_mems[conv_name] = unitmem_neuron_dict

        data = list(zip(map(float, selectivity), map(int, maxposition)))
